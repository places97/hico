...
### eclipse 만 해당 > maven update 후 서버 실행 안 될때 :
1. project 우클릭 -> properties -> Project Facet -> Dynamic Web Module 3.1 로 변경

### Maven install
1. .m2 폴더 하위에 resource/settings/settings.xml 파일을 복사하여 붙여놓고 install
2. 인식하지 못할 경우 maven settings path가 수정되었는지 확인 후 해당 경로로 변경
